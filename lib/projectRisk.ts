import { ProjectProfile, ProjectStage, StackTag, STAGE_ORDER } from './project'
...
export const RISK_DATABASE: Risk[] = [
...

  {
    ...
    why: 'Most users will immediately abandon your app when they see the Google unverified warning. And 100 users is a hard ceiling — user 101 cannot sign in at all. This is one of the most common silent launch-blockers for vibe coders.',
    aiPrompt: 'I need to publish my Google OAuth app so it works for unlimited users without showing the unverified warning. Walk me through: (1) Go to Google Cloud Console → APIs & Services → OAuth consent screen, (2) Click "Publish App" to move from Testing to Production, (3) Confirm what scopes I am requesting and whether they require Google verification, (4) If I am requesting sensitive scopes (email, profile are not sensitive — Drive, Calendar are), explain what verification is required. Tell me exactly what I need to fill in to complete the publication.',
    source: '@authexpert · verified',
  },
  ...
  },
  {
    ...
    why: 'GDPR and ePrivacy Directive require informed consent before non-essential cookies. Fines have been issued to small apps. Google Analytics without consent is a liability.',
    aiPrompt: 'Add a cookie consent banner to my app that fires before any analytics tracking. It should: offer Accept/Decline options, not fire analytics scripts until accepted, store the consent decision, and link to my Privacy Policy. Show me the implementation for my framework.',
    source: '@startuplawyer · verified',
  },
  {
    ...
    requiresUserData: true,
    minStage: 'pre-launch',
    problem: 'COPPA (US) prohibits collecting data from children under 13 without verifiable parental consent. Many apps accidentally get this wrong.',
    why: 'FTC has issued fines of $5M+ to apps that violated COPPA. If your app could appeal to children and you collect any data, you need an age gate.',
    aiPrompt: 'Add an age verification step to my signup flow that prevents users under 13 from creating accounts. Add an age check field (date of birth or simple 13+ confirmation) before account creation. Store the confirmation. Update my Privacy Policy to include a COPPA section. Show me the implementation.',
    source: '@startuplawyer · verified',
  },
  {
    ...
    aiPrompt: 'Generate a Refund and Cancellation Policy for my app. Include: digital product refund window (or no-refund justification), subscription cancellation terms, how to request a refund, and processing time. Add it to my app settings screen and link it from the checkout flow. Show me where to add the links.',
    source: '@startuplawyer · verified',
  },
  {
    ...
    minStage: 'pre-launch',
    problem: 'Apple and Google require you to declare exactly what data your app collects in your store listing. Missing or inaccurate labels can get your app rejected or pulled.',
    why: 'Apple has rejected apps for inaccurate privacy labels. If you collect emails, usage data, or identifiers, every category must be declared accurately.',
    aiPrompt: 'Audit my app for every category of data it collects: identifiers (user ID, device ID), usage data (analytics), contact info (email, name), location, and diagnostics. For each category, tell me how to accurately declare it in App Store Connect and Google Play Console. Show me a completed privacy label checklist for my app.',
  ...
  // ─── INFRASTRUCTURE ──────────────────────────────────────────────────────────
  {
    ...
    why: 'Without Sentry or similar, you are flying blind after launch. Silent errors erode retention and trust without you ever knowing they happened.',
    aiPrompt: 'Add Sentry to my project for error monitoring. Install the correct SDK for my platform (sentry-expo for Expo, @sentry/nextjs for Next.js, sentry-sdk for FastAPI). Initialize it with my DSN, capture unhandled exceptions, and set the environment to "production" on deploy. Show me the setup for both frontend and backend.',
    source: '@infrahardening · verified',
  },
  {
    id: 'no-db-backups',
    ...
    why: 'A single bad migration or accidental delete can wipe everything. Backups are the difference between an incident and a catastrophe.',
    aiPrompt: 'Help me set up automated backups for my database. Tell me what my provider offers (point-in-time recovery, daily snapshots), how to enable it, and how to test a restore. Add a short runbook to my repo describing how to recover from data loss.',
    source: '@dbsecurity · verified',
  },
  {
    ...
    stackAny: ['fastapi', 'node', 'nextjs'],
    minStage: 'pre-launch',
    problem: 'Without a /health or /ping endpoint, uptime monitors, load balancers, and deployment platforms cannot verify your service is running.',
    why: 'Railway, Fly.io, and AWS all use health checks to route traffic. Without one, you cannot use zero-downtime deploys or uptime monitoring.',
    aiPrompt: 'Add a GET /health endpoint to my API that returns { "status": "ok", "timestamp": "<now>" } with HTTP 200. Add a more detailed version that checks database connectivity and returns 503 if the DB is unreachable. Show me the implementation.',
    source: '@infrahardening · verified',
  },
  {
    id: 'no-staging-env',
    ...
    severity: 'medium',
    minStage: 'pre-launch',
    problem: 'Every change goes directly to production with no testing environment. A broken deploy takes down live users immediately.',
    why: 'A 10-minute outage on launch day can permanently damage trust. Staging costs almost nothing and prevents "it worked locally" disasters.',
    aiPrompt: 'Help me set up a staging environment that mirrors my production stack. Show me how to: create a staging branch, deploy it to a separate URL, connect a staging database, and set ENVIRONMENT=staging so I can distinguish staging from production in my code. Show the config for my hosting provider.',
    source: '@infrahardening · verified',
  },
  {
    id: 'missing-env-docs',
    ...
    severity: 'medium',
    minStage: 'building',
    problem: 'There is no .env.example or documented list of required environment variables. New contributors (or future you after a fresh clone) cannot set up the project.',
    why: 'Missing env vars cause confusing runtime errors that take hours to debug. A .env.example is a 5-minute fix that saves hours.',
    aiPrompt: 'Scan my codebase for every place an environment variable is accessed (process.env.*, os.environ.get, etc). Create a .env.example file listing every required variable with a placeholder value and a one-line comment describing what it is. Add validation at startup that throws a clear error if any required variable is missing.',
    source: '@infrahardening · verified',
  },
  {
    ...
    severity: 'high',
    stackAny: ['openai'],
    problem: 'Your app calls OpenAI (or another AI API) with no usage limits or spend caps. A bot, a viral moment, or a bug loop can run up thousands of dollars in minutes.',
    why: 'There are documented cases of developers waking up to $10,000+ AI API bills from a single incident. Spend caps are free to set and protect you.',
    aiPrompt: 'Help me add cost protection to my AI API usage: (1) Set a monthly spend cap in my OpenAI/Anthropic account dashboard — show me where to find it. (2) Add per-user request limits in my code so no single user can make more than N AI calls per day. (3) Add a server-side check before each AI call that verifies the user hasn\'t exceeded their quota. Show me the implementation.',
    source: '@infrahardening · verified',
  },
  ...
  // ─── PAYMENTS ────────────────────────────────────────────────────────────────
  {
    ...
    requiresPayments: true,
    minStage: 'building',
    problem: 'If a payment API call is retried (network timeout, user double-taps), you can create duplicate charges for the same order.',
    why: 'Double charges generate chargebacks and destroy trust. Idempotency keys are a one-line fix that makes payment creation safe to retry.',
    aiPrompt: 'Find every place I create a Stripe PaymentIntent, charge, or subscription. Add an idempotency key to each call — generate it from a stable identifier like orderId or `${userId}-${productId}-${timestamp}`. Show me the updated API calls with idempotency keys added.',
    source: '@stripepro · verified',
  },
  {
    ...
    requiresPayments: true,
    minStage: 'pre-launch',
    problem: 'Every refund requires you to manually process it through the Stripe dashboard. This does not scale and creates support bottlenecks.',
    why: 'When you have 100+ customers, manual refunds take hours. A self-serve refund flow reduces support load and improves customer trust.',
    aiPrompt: 'Add a refund request flow to my app. Users should be able to: find a past payment in their account, click "Request refund", optionally provide a reason, and submit. On the backend, create an endpoint that calls Stripe\'s refund API and updates the order status. Send a confirmation email. Show me the full implementation.',
    source: '@stripepro · verified',
  },
  {
    ...
    requiresPayments: true,
    minStage: 'building',
    problem: 'When a payment fails (declined card, insufficient funds, expired card), users see a generic error and have no idea what to do next.',
    why: 'Poor payment failure UX causes ~20% of users to abandon instead of retrying with a different card. Every abandoned failure is lost revenue.',
    aiPrompt: 'Improve my payment failure handling. For each Stripe error code (card_declined, insufficient_funds, expired_card, etc) show a specific user-friendly message that tells them exactly what to do. Add a "try another card" button. Log failed payment attempts with the error reason. Show me the updated error handling code.',
    source: '@stripepro · verified',
  },
  {
    ...
    stackAny: ['expo', 'react-native'],
    minStage: 'pre-launch',
    problem: 'Apple requires that users can cancel subscriptions from within the app. Apps that force users to go to Settings → Apple ID are rejected.',
    why: 'This is a common App Store rejection reason for subscription apps. Apple\'s guideline 3.1.2 requires in-app cancellation. Google has the same requirement.',
    aiPrompt: 'Add a subscription management screen to my app that shows the current plan, billing date, and a Cancel Subscription button. On iOS, link to the Apple subscription management URL. On Android, link to the Google Play subscription management URL. Show me the implementation using Linking.openURL for each platform.',
    source: '@appstoreexpert · verified',
  },
  ...
  // ─── APP STORE / MOBILE ──────────────────────────────────────────────────────
  {
    ...
    stackAny: ['expo', 'react-native'],
    minStage: 'pre-launch',
    problem: 'Apple requires a PrivacyInfo.xcprivacy file for any app using APIs that access device data (file timestamps, user defaults, etc). This became mandatory in Expo SDK 52+.',
    why: 'Apps without the privacy manifest are rejected at App Store review. This is a hard blocker that has surprised many vibe coders right before launch.',
    aiPrompt: 'Add the required iOS privacy manifest to my Expo project. Create a PrivacyInfo.xcprivacy file in my project root that declares all required API usage reasons. For a typical Expo app, include: NSPrivacyAccessedAPICategoryFileTimestamp, NSPrivacyAccessedAPICategoryUserDefaults, and NSPrivacyAccessedAPICategoryDiskSpace. Show me the complete file and where to place it in my Expo project.',
    source: '@appstoreexpert · verified',
  },
  {
    ...
    stackAny: ['expo', 'react-native'],
    minStage: 'pre-launch',
    problem: 'Every iOS permission (camera, contacts, location, microphone, photos) requires a human-readable description of WHY you need it. Missing descriptions cause App Store rejection.',
    why: 'Apple rejects apps that request permissions without descriptions. The description shows to users in the system permission dialog — vague descriptions also cause rejections.',
    aiPrompt: 'Check my app.json or app.config.js for missing iOS permission descriptions. For every permission my app requests, add a clear, user-facing description of why it is needed. Use specific reasons like "We need camera access to let you scan documents" rather than "Required for app functionality". Show me the complete updated permissions config.',
    source: '@appstoreexpert · verified',
  },
  {
    ...
    stackAny: ['expo'],
    minStage: 'building',
    problem: 'Your app is running on an Expo SDK version that is more than one major version behind. Expo drops support for old SDKs, and old SDKs have known security vulnerabilities.',
    why: 'Old Expo SDKs stop receiving updates and can block App Store submissions as Apple raises minimum iOS requirements. Staying current also gives you access to new native APIs.',
    aiPrompt: 'Check my current Expo SDK version in package.json. If it is not the latest stable, show me the step-by-step upgrade path including: running npx expo install --fix, checking for breaking changes in the Expo changelog, updating native dependencies, and testing on both iOS and Android simulators. List any breaking changes I need to address.',
    source: '@appstoreexpert · verified',
  },
  {
    ...
    stackAny: ['expo', 'react-native'],
    minStage: 'pre-launch',
    problem: 'When your app crashes on a user\'s device, you have no record of it. You depend entirely on users reporting crashes manually.',
    why: 'Most users who experience a crash just uninstall. Without crash reporting, you cannot fix what you cannot see.',
    aiPrompt: 'Add Sentry crash reporting to my Expo/React Native app. Install @sentry/react-native, initialize it in App.js with my DSN and set environment to "production", wrap my root component with Sentry.wrap(), and enable native crash reporting. Show me the complete setup including how to test that crashes are being captured.',
    source: '@appstoreexpert · verified',
  },
  {
    ...
    stackAny: ['expo'],
    minStage: 'pre-launch',
    problem: 'Expo OTA (over-the-air) updates go directly to all production users. A bad update can break the app for every user simultaneously, with no way to test first.',
    why: 'Unlike App Store updates, OTA updates have no review process. A broken OTA update hits 100% of your users the moment they open the app.',
    aiPrompt: 'Set up Expo update channels so I can test OTA updates before they reach production users. Create a "staging" channel in eas.json that goes to TestFlight/internal testers, and a "production" channel for all users. Update my eas.json to configure both channels. Show me how to publish to staging first, verify it works, then promote to production.',
    source: '@appstoreexpert · verified',
  },
]
